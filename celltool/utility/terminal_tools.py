"""Tools for fancy-formatting terminal output.

This module includes classes for displaying and updating progress bars in
terminal windows that support such fancy operations.

These tools are based on Edward Loper's Python Cookbook recipe:
...
class ProgressBar:
    ...
    def fancy_update(self, percent, message):
        ...
        sys.stdout.write(
            ...
            (self.bar % (100*percent, '='*(n - 1), '>', '-'*(self.width-10-n))) +
...
def progress_list(input_list, header, element_namer = lambda x: x):
    ...
    """
    pb = ProgressBar(header)
    l = list(input_list)
    length = len(l)
    for i, element in enumerate(l):
        pb.update((i + 1.0) / length, str(element_namer(element)))
        yield element
...
class IndeterminantProgressBar:
    ...
    def simple_update(self, message):
        sys.stdout.write(message + '\n')

    def fancy_update(self, message):
        if len(message) > self.width:
            message = '...' + message[len(message) - self.width + 5:]
        if self.cleared:
            sys.stdout.write(self.header)
            self.cleared = 0
        n = self.width-10
        line = ''.join([self.SYMBOLS[(i + self.phase)%2] for i in range(n)])
        sys.stdout.write(
            self.term.BOL + self.term.UP + self.term.CLEAR_EOL +
            (self.bar % line) + self.term.CLEAR_EOL + message.center(self.width) )
        self.phase += 1
        sys.stdout.flush()
